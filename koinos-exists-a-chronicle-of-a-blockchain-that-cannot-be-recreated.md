...
26. **Infrastructure decentralization and experimentation:** `api.koinos.io` moves from company and individually subsidized services toward a Cloudflare load balancer with multiple node operators, `pgarcgo` makes the public testnet reproducible, and `@interfecto` explores EVM compatibility on that testnet.
27. **Alternative-client proof:** Knodel grows from a local explorer and recovery tool into Koinos One and Teleno; on July 12, 2026, the community-led compatible node path synchronizes with mainnet and produces accepted blocks while preserving an observer-first safety procedure.
28. **Upstream maintenance and rebuild signals:** Kondor v2 returns through Armana after Julián's final v1 modernization work, with Ron handling the public beta path; Julián's producer-throttling change reduces concentration risk; and `pgarcgo`'s reviewed state-database and chain patches in `koinos-chain v1.5.2` turn a 37-million-block audit and operator failures into safer, verifiable fast synchronization.
